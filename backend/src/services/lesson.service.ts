import type { LessonRecord, LessonDetail, LessonMessage } from '../types/lesson.types';
...
const lessonDetailsMap: Record<number, LessonDetail> = {
  1: {
    id: 1,
    courseName: '机器学习入门',
    title: '第 1 课：什么是机器学习',
...
const lessonGraphsMap: Record<number, LessonGraphData> = {
  1: {
    nodes: [
...
export function getLessonDetail(lessonId: number): LessonDetail | null {
  return lessonDetailsMap[lessonId] ?? null;
}

export function getLessonGraph(lessonId: number): LessonGraphData | null {
  return lessonGraphsMap[lessonId] ?? null;
}
